...
import com.happytrip.exception.DuplicateRouteException;
...
public class AdminServiceImpl implements AdminService {
	...
	@Override
	public boolean isDuplicateRoute(String fromCity, String toCity) throws DuplicateRouteException {
		//Route route = routeDao.findByCityNames(fromCity, toCity);
		List<Route> routes = routeDao.getAllRoutes();
		boolean flag = false;
		for(Route route1 : routes){
			if(route1.getToCity().getCityName().equalsIgnoreCase(toCity) && route1.getFromCity().getCityName().equalsIgnoreCase(fromCity)){
				flag = true;
	...
	public void setScheduleFlightDetails(FlightScheduleDto flightScheduleDto,
		...
		if (flightScheduleDto.getScheduledFlight().getDistanceInKms() >16000) {
